...
<template>
  <div>
    <button
      ...
      class="py-3.5 -my-3.5 text-xs text-black/40 hover:text-accent"
      @click="start"
    ...
    <NuxtLink
      ...
      class="text-xs text-black/40 hover:text-accent"
      ...
      <div class="w-full max-w-sm rounded-lg bg-white p-5 shadow-xl">
        <template v-if="!done">
          ...
          <div class="mt-3 space-y-1.5">
            <label
              ...
              class="flex cursor-pointer items-center gap-2 rounded px-2 py-2 text-sm hover:bg-black/[0.03]"
            >
          ...
          <label class="mt-3 block">
            <span class="text-xs font-medium text-black/60">
              {{ $t('song.reportNote') }}
              <span v-if="kind !== 'other'" class="font-normal text-black/35">{{ $t('page.optional') }}</span>
            </span>
            <textarea
          ...
          <div class="mt-4 flex justify-end gap-2 text-sm">
            <button class="rounded px-3 py-1.5 text-black/60 hover:text-accent" @click="open = false">
              {{ $t('song.cancel') }}
            </button>
            <button
              class="rounded bg-accent px-3 py-1.5 text-white disabled:opacity-40"
              :disabled="!canSend"
              @click="send"
        ...
        <template v-else>
          ...
          <div class="mt-4 flex justify-end">
            <button class="rounded bg-accent px-3 py-1.5 text-sm text-white" @click="open = false">
              {{ $t('common.close') }}
            </button>
          </div>
        </template>
