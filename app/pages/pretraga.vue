...
<template>
  ...
  <p v-if="status === 'pending'" class="text-sm text-black/50">Traženje…</p>
  <p v-else-if="!hasResults" class="text-sm text-black/50">Ništa nije pronađeno.</p>

  <template v-else>
    <section v-if="data.artists?.length" class="mb-8">
      <h2 class="mb-2 text-sm font-semibold uppercase tracking-wide text-black/40">Izvođači</h2>
    ...
    <section v-if="data.genres?.length" class="mb-8">
      <h2 class="mb-2 text-sm font-semibold uppercase tracking-wide text-black/40">Rubrike</h2>
      <ul class="flex flex-wrap gap-2">
    ...
    <section v-if="data.songs?.length">
      <h2 class="mb-2 text-sm font-semibold uppercase tracking-wide text-black/40">Pjesme</h2>
