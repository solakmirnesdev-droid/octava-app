...
<template>
  <div class="flex min-h-screen items-center justify-center bg-surface px-6 text-ink">
    <div class="w-full max-w-sm">
      ...
      <p v-if="!token" class="mt-6 rounded bg-accent-soft px-4 py-3 text-sm text-accent">
        {{ $t('auth.linkIncomplete') }}
      </p>

      <template v-else-if="!done">
        <p class="mt-1 mb-8 text-sm text-muted">{{ $t('auth.setNewPassword') }}</p>
        ...
        <form @submit.prevent="submit">
          ...
          <p v-if="error" role="alert" class="mt-4 rounded bg-accent-soft px-3 py-2 text-sm text-accent">
            {{ error }}
          </p>
      ...
      <p v-if="!done" class="mt-6 text-center text-sm text-muted">
